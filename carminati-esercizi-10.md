...
private:
  std::uint32_t m_a, m_c;
  std::uint32_t m_seed;
};
```

Il costruttore deve accettare un intero senza segno a 32 bit come *seed* di input e inizializzare i parametri del generatore ai valori nominali `m_a = 1664525`, `m_c = 1013904223`. Avremmo in teoria dovuto anche includere il parametro `m_m = 1UL << 32`, ma ciò è inutile: usando interi a 32 bit, è ovvio che ogni operazione sarà il risultato di un modulo 2³². È possibile usare altri valori per i parametri, come spiegato nella [pagina Wikipedia](https://en.m.wikipedia.org/wiki/Linear_congruential_generator#Parameters_in_common_use), ed in quel caso si dovrebbe effettivamente introdurre anche un parametro `m_m`, se questo fosse diverso da 2³²; ma vi consiglio vivamente di limitarvi ad usare questi numeri ed ignorare il modulo, in modo da poter confrontare i vostri risultati con quelli del [notebook Julia](https://ziotom78.github.io/tnds-notebooks/lezione10/#esercizio_100).

**Attenzione!** È indispensabile usare un tipo *unsigned* come `std::uint32_t` nell’implementazione, perché il generatore lineare congruenziale si appoggia al fatto che l’aritmetica dei tipi *unsigned* è una [aritmetica modulare](https://en.wikipedia.org/wiki/Modular_arithmetic): se si aumenta di uno una variabile che contiene il valore massimo, la variabile viene impostata a zero (*wrap-around*). Si tratta di un meccanismo simile a quello degli angoli in una circonferenza: quando si arriva a 359° e si incrementa di 1°, si ottiene di nuovo l’angolo 0°. Il tipo `std::uint32_t` usa come modulo del *wrap-around* il numero $2^{32}$, il tipo `std::uint8_t` usa $2^8$, il tipo `std::uint64_t`  usa $2^{64}$, e così via: è proprio per questo che l’operazione di modulo `m` (2³²) non va scritta nel codice, perché implementata implicitamente dall’uso di questo tipo.

L’aritmetica modulare su valori $2^{32}$ è esattamente ciò su cui si affida l’algoritmo di generazione dei numeri pseudocasuali che implementiamo oggi. Questo comportamento *non* è vero per gli interi con segno come `int` e `std::int32_t` (per cui lo standard C++ non prescrive nulla: è un [*undefined behavior*](https://en.cppreference.com/w/cpp/language/ub.html)).

## Tipi di dimensione fissata

Come abbiamo detto sopra, l’algoritmo di generazione fornito in classe è pensato per lavorare con numeri a 32 bit. Sulle macchine Linux e Windows moderne, il tipo `unsigned int` (così come `int`) è proprio di questa dimensione, ma lo standard C++ non garantisce ciò su **qualsiasi** macchina! Esistono infatti computer dove `unsigned int` ed `int` occupano 16 bit, soprattutto in sistemi *embedded* (schede elettroniche, montate ad esempio dentro strumenti scientifici).

Per essere più sicuri della portabilità del codice, anziché usare `unsigned int` è consigliabile usare il tipo `std::uint32_t` (definito in `<cstdint>`). Esso è definito solo su macchine che supportano numeri a 32 bit, e in quel caso è **sempre** a 32 bit. Siccome alcuni sistemi non sono in grado di gestire numeri a 32 bit, in quei casi il nostro codice dovrebbe fallire la compilazione anziché produrre un eseguibile che fa calcoli sbagliati!
...
- `int8_t` indica che si vuole un tipo di **esattamente** 8 bit. Se il compilatore C++ non lo supportasse, `int8_t` potrebbe non essere definito (in passato sono esistite macchine che usavano 7 o 9 bit per i caratteri!): in tal caso, un codice che lo usasse **non compilerebbe**. Questo non è uno svantaggio, ma una garanzia di sicurezza: siamo certi che il nostro codice verrà usato solo su macchine appropriate!
- `int_least8_t` indica che si desidera un tipo che contenga **almeno** 8 bit, evidentemente perché serve per memorizzare numeri positivi o negativi che potrebbero raggiungere al massimo il valore di un centinaio o poco più (un intero a 8 bit può contenere numeri nell’intervallo −128…127). Se la macchina fosse un microcontrollore a 16 bit, probabilmente `int_least8_t` sarebbe un alias a `int16_t`, che è il tipo nativo del microcontrollore.
- `int_fast8_t` indica che si vuole usare un tipo che abbia almeno 8 bit, e che sia il tipo **più veloce** sulla macchina a disposizione. Non è infatti detto che un tipo con pochi bit sia più veloce da usare di un tipo con molti bit, perché dipende dall’architettura della CPU e della memoria RAM Ad esempio, sulle macchine del laboratorio, `int_fast8_t` è probabilmente un alias a `int32_t`. I tipi `*_fast_t` dovrebbero essere usati quando la velocità è massimamente importante, anche al prezzo di usare magari più memoria di quanto necessaria.
